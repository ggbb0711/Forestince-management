...
function BookingDetailContent({ promise }: { promise: Promise<FacilityBooking> }) {
...
export function BookingDetailPage() {
  ...
  const bookingPromise = useMemo(() => getBookingById(bookingId!), [bookingId])
    ...
    <div className="flex-1 flex flex-col overflow-hidden">
      <div className="flex-1 overflow-auto p-4 lg:p-5">
        ...
        <Suspense fallback={<BookingDetailSkeleton />}>
          <BookingDetailContent promise={bookingPromise} />
        </Suspense>
      </div>
    </div>
  )
